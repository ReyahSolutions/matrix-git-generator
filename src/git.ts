...
export async function getAllFilesForDepth(depth: number): Promise<string[]> {
  let buffer = '';

  const code = await exec('git', ['ls-tree', '-r', '--name-only', 'HEAD'], {
    listeners: {
      stdout: (data) => (buffer += data.toString())
    }
  });
  if (code !== 0) {
    throw new Error(`Couldn't determine files`);
  }
